...
\begin{lstlisting}
// hash mapeia uma chave (string) para um indice do array.
...
func (h *HashTable) Put(key string, value int) {
    ...
    for e := h.buckets[idx]; e != nil; e = e.next {
        if e.key == key {
            e.value = value // chave ja existe: atualiza
            return
        }
    }
...
// Get retorna o valor associado a chave e se foi encontrada.
...
func (h *HashTable) IsEmpty() bool { return h.size == 0 }
\end{lstlisting}
...
\begin{lstlisting}
// higherPriority: em min-heap a < b; em max-heap a > b.
...
// siftUp sobe o elemento i enquanto tiver prioridade maior que o pai.
...
// siftDown desce o elemento i restaurando a propriedade de heap.
